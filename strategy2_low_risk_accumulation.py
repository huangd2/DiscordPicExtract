...
# Configuration
STRATEGY_NAME = "Low Risk Accumulation Strategy 2 - Progressive Buy"
...
BUY_CRITERIA = {
    ...
    6: (2.5, 4),   # 6th buy: >=2.5% drop, 4 shares
    7: (3.0, 4),   # 7th buy: >=3.0% drop, 4 shares
}

import os
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
CSV_FILE = os.path.join(SCRIPT_DIR, 'combined_data.csv')
STRATEGY2_DIR = os.path.join(SCRIPT_DIR, 'strategy2')
os.makedirs(STRATEGY2_DIR, exist_ok=True)
OUTPUT_CSV = os.path.join(STRATEGY2_DIR, 'strategy2_low_risk_accumulation_trades.csv')
...
def get_buy_criteria(buy_signal_num):
    """Get buy criteria for buy signal number. Continues pattern after 7th buy."""
    if buy_signal_num in BUY_CRITERIA:
        return BUY_CRITERIA[buy_signal_num]
    elif buy_signal_num > 7:
        # Continue pattern: 3.5%, 4.0%, etc. with 4 shares each
        drop_pct = 3.0 + (buy_signal_num - 7) * 0.5
        return (drop_pct, 4)
    return None
    ...
    if signal_type == 'Buy' and risk == 'low':
        if shares == 0:
            ...
            if cash >= f_price:
                ...
                position_cost_basis = f_price  # Track cost basis
            ...
            if first_buy_price is not None:
                ...
                # else: No criteria for this buy signal number or price drop doesn't meet criteria, skip
...
# For final position, only sell if price > avg buy price (follow the rule)
if shares > 0:
    ...
    # Only sell if final price > avg buy price
    ...
    if signal_type == 'Buy' and risk == 'low':
        if shares == 0:
            ...
            if first_buy_price is not None:
                ...
                # else: Price drop doesn't meet criteria, skip
        # Track missed buys if we couldn't afford first buy
        if shares == 0 and cash < f_price:
            daily_missed_buys[signal_date] += 1
...
# Handle final position (for daily stats - only if sold)
if shares > 0:
    last_signal = df.iloc[-1]
    final_price = last_signal['fPrice']
    avg_buy_price = position_cost_basis / shares if shares > 0 else 0
...
STATS_CSV = os.path.join(STRATEGY2_DIR, 'strategy2_statistics.csv')
stats_data = {
    'Metric': [
        'Initial Capital',
...
plt.tight_layout()
plt.savefig(os.path.join(STRATEGY2_DIR, 'trades_per_day_histogram.png'), dpi=300, bbox_inches='tight')
print(f"  Saved: {os.path.join(STRATEGY2_DIR, 'trades_per_day_histogram.png')}")
plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(STRATEGY2_DIR, 'daily_pnl_histogram.png'), dpi=300, bbox_inches='tight')
print(f"  Saved: {os.path.join(STRATEGY2_DIR, 'daily_pnl_histogram.png')}")
plt.close()
...
# Handle final position if still open (sell at final price for equity curve)
if running_shares > 0:
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(STRATEGY2_DIR, 'equity_curve.png'), dpi=300, bbox_inches='tight')
print(f"  Saved: {os.path.join(STRATEGY2_DIR, 'equity_curve.png')}")
plt.close()
...
if total_trades > 0:
    ...
    plt.axis('equal')
    plt.tight_layout()
    plt.savefig(os.path.join(STRATEGY2_DIR, 'trade_win_loss.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: {os.path.join(STRATEGY2_DIR, 'trade_win_loss.png')}")
    plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(STRATEGY2_DIR, 'daily_pnl_over_time.png'), dpi=300, bbox_inches='tight')
print(f"  Saved: {os.path.join(STRATEGY2_DIR, 'daily_pnl_over_time.png')}")
plt.close()
...
plt.grid(True, alpha=0.3, axis='y')
plt.tight_layout()
plt.savefig(os.path.join(STRATEGY2_DIR, 'daily_positions.png'), dpi=300, bbox_inches='tight')
print(f"  Saved: {os.path.join(STRATEGY2_DIR, 'daily_positions.png')}")
plt.close()
