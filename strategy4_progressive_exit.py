...
# Configuration
STRATEGY_NAME = "Low Risk Accumulation Strategy 4 - Progressive Exit with Trailing Stop-Loss"
...
OUTPUT_CSV = os.path.join(STRATEGY4_DIR, 'strategy4_progressive_exit_trades.csv')
...
sell_stage = 0  # 0 = no sells, 1 = first sell done (50%), 2 = second sell done (25%), 3 = all sold
    ...
    # Handle Sell signals (any risk level, as long as price > avg buy price)
        ...
        if shares > 0:
            ...
            # Sell if price > avg buy price (and stop-loss not triggered)
                ...
                if sell_stage == 0:
                    # First sell: sell 50% of position
                    ...
                    # Second sell: check for buy signals between sells, then sell half of remaining position (round up)
                    if not has_lower_buy_since_last_sell(df, last_sell_timestamp, timestamp, avg_buy_price):
                        # Calculate half of remaining position, rounding up
                        shares_to_sell = (shares + 1) // 2
                    ...
                    # Third sell: check for buy signals between sells, then sell remaining 25%
                    if not has_lower_buy_since_last_sell(df, last_sell_timestamp, timestamp, avg_buy_price):
                        # Sell all remaining shares (should be ~25% of original)
...
if shares > 0:
    ...
    # Only sell if final price > avg buy price (follow the rule)
        ...
        if sell_stage == 0:
            ...
            # Second sell: sell half of remaining position, rounding up
            shares_to_sell = (shares + 1) // 2
            if shares_to_sell > 0:
                cost_basis_sold = (position_cost_basis * shares_to_sell) / shares
                proceeds = final_price * shares_to_sell
...
STATS_CSV = os.path.join(STRATEGY4_DIR, 'strategy4_statistics.csv')
stats_data = {
    'Metric': [
        'Initial Capital',
        ...
        if signal_type == 'Buy' and risk == 'low':
            ...
            if running_shares > 0:
                ...
                # Sell if price > avg buy price (and stop-loss not triggered)
                    ...
                    if sell_stage == 0:
                        # First sell: 50%
                        shares_to_sell = running_shares // 2
                        if shares_to_sell > 0:
                        ...
                        # Second sell: check for buy signals between sells, then sell half of remaining position (round up)
                        if not has_lower_buy_since_last_sell(df, last_sell_timestamp, timestamp, avg_buy_price):
                            shares_to_sell = (running_shares + 1) // 2
                            if shares_to_sell > 0:
                                cost_basis_sold = (position_cost_basis * shares_to_sell) / running_shares
                                proceeds = sell_price * shares_to_sell
                                running_cash += proceeds
