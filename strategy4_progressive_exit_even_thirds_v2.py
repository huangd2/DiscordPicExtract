...
# Configuration
STRATEGY_NAME = "Low Risk Accumulation Strategy 4.2 - Progressive Exit (Even Thirds) with Avg Price Exit"
...
STRATEGY4_DIR = os.path.join(SCRIPT_DIR, 'strategy4.2')
os.makedirs(STRATEGY4_DIR, exist_ok=True)
OUTPUT_CSV = os.path.join(STRATEGY4_DIR, 'strategy4_progressive_exit_even_thirds_v2_trades.csv')
...
for idx, row in df.iterrows():
    ...
    if signal_type == 'Buy' and risk == 'low':
        if shares == 0:
            ...
            if first_buy_price is not None:
                ...
                # Check for buy signal reset: only reset if no sells yet
                # After first sell, price < avg_buy_price triggers exit (handled above)
    ...
    # Handle Sell signals (any risk level, as long as price > avg buy price)
        ...
        if shares > 0:
            ...
            # Check stop-loss first (especially for sell_stage == 1 and 2)
            stop_loss_triggered = False
            if sell_stage == 1:
                # After 1st sell: check both 1.5% stop-loss and avg_buy_price exit
                stop_loss_price = avg_buy_price * (1 - STOP_LOSS_PCT / 100)
...
STATS_CSV = os.path.join(STRATEGY4_DIR, 'strategy4_v2_statistics.csv')
stats_data = {
    'Metric': [
        'Initial Capital',
